...
export class GetConversationsByTenantHandler implements IQueryHandler<
...
> {
  ...
  async execute(
    query: GetConversationsByTenantQuery,
  ): Promise<GetConversationsByTenantResult> {
    const filters: ConversationFilters = {};

    if (
      query.channel &&
      Object.values(GuestMessageChannel).includes(
        query.channel as GuestMessageChannel,
      )
    ) {
    ...
    const { conversations, total } =
      await this.conversationRepository.findByTenantPaginated(
        query.tenantId,
        filters,
        query.page,
        query.limit,
